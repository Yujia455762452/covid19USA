...
setwd("C:\\Users\\ysl\\Desktop\\data")
# COVID DATA
df <- read.csv("national-history.csv",header = TRUE,sep = ",",quote = '"') 
data <- df
...
stock_ymd <- read.csv("GOOG_stock.csv",header = TRUE,sep = ",",quote = '"') 
stock_ymd$date <- lubridate::ymd(stock_ymd$date)
...
# map data
American_map <-read.csv("American_map_data.csv")
# map cloor
map_color <-read.csv("covid_20201001.csv")
